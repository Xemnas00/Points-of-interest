...
#Function that drops rows having inactive POIs
def filter_open_places_sicilia(data_frame):
    data_frame["Orari"] = data_frame["Orari"].map(str.lower)
    #REGEX because sometimes "chiuso" or "aperto" doesn't mean that POI is always closed or open but only in a specific day
    ...
    for i in range(data_frame["Orari"].size):
        ...
        #"APERTO" doesn't match pattern2 so cannot be modified
...
#Function that drops useless columns and renames others
...
#Function that assigns the right price to the "price column"
def fix_prices_sicilia(data_frame):
    ...
    data_frame["Prezzo"] = data_frame["Prezzo"].str.replace("€", "")
    data_frame["Prezzo"] = data_frame["Prezzo"].str.replace(",", ".")
    #Trying to cast "price column" to float
    try:
...
#Function that recognize POI categories (museum, archaeological area or monumental area)
...
#Function that fixes addresses so that geopy will be able to retrieve latitude and longitude
def fix_addresses_sicilia(data_frame):
    data_frame["Indirizzo"] = data_frame["Indirizzo"].map(str.lower)
    #Selecting well-formed addressess
    pattern = re.compile("(via|viale|piazza|corso|piazzetta|lungomare)")
    pattern2 = re.compile("[a-z]+(,|\))*")  #')' is important for " Via Patti)"
    for i in range(data_frame["Indirizzo"].size):
        #Splitting using whitespaces
        vett = data_frame["Indirizzo"].values[i].split()
        result = ""
        for j in range(len(vett)):
            if pattern.match(vett[j]):  #Found a word that matches the pattern and so we will looking for next words (the address name)
                result = vett[j]
                j += 1  #Visiting next word
                while j<len(vett) and pattern2.match(vett[j]) != None:  #We don't exceed list dimension and the found word matches pattern2
                    result += " " + vett[j] #Concatenating
                    j += 1
        result = result.replace(",", "") #Deleting ',' characters
        result = result.replace(")", "") #Deleting ')' characters

        if result != "":    #We found a valid address
            data_frame["Indirizzo"].values[i] = result + ", " + data_frame["Comune"].values[i] +", Italia"
        else:   #We didn't found a valid address so inserting "city" + ", Italia"
...
#Function that adds latitude and longitude using geopy and directly fixes some values
...
#Function that extracts phone numbers by contacts column
def fix_phone_numbers_sicilia(data_frame):
    for i in range(data_frame["Contatti"].size):
        ...
        data_frame["Contatti"].values[i] = data_frame["Contatti"].values[i].replace(" ", "")
        #Finding all substrings that match the pattern below
